...
parser.add_argument(
    "--terrain_override_file",
    type=str,
    default=None,
    help="JSON terrain override file generated by the training panel for run-faithful playback.",
)
parser.add_argument(
    "--camera_follow_robot",
    action="store_true",
    default=False,
    help="Make the viewport camera follow the robot asset during playback/video recording.",
)
parser.add_argument(
    "--camera_eye",
    type=float,
    ...
    default=(-3.0, -2.4, 1.6),
    metavar=("X", "Y", "Z"),
    help="Camera eye offset used with --camera_follow_robot.",
)
parser.add_argument(
    "--camera_lookat",
    type=float,
    ...
    metavar=("X", "Y", "Z"),
    help="Camera target offset used with --camera_follow_robot.",
)
parser.add_argument(
    "--export_policy",
    action="store_true",
...
def _apply_panel_terrain_override(env_cfg, override_file: str | None) -> None:
    ...
    if not path.exists():
        print(f"[WARN] Training panel terrain override file not found: {path}")
        return
    try:
        ...
        applied = apply_terrain_overrides(env_cfg, overrides)
        source = payload.get("source", str(path)) if isinstance(payload, dict) else str(path)
        if applied:
            print(f"[INFO] Training panel terrain replay applied from {source}: {', '.join(applied)}")
        else:
            print(f"[WARN] Training panel terrain override file had no applicable keys: {path}")
    except Exception as exc:
        print(f"[WARN] Failed to apply training panel terrain override file '{path}': {exc}")


def _configure_follow_camera(env_cfg) -> None:
    if not args_cli.camera_follow_robot:
        return
    viewer = getattr(env_cfg, "viewer", None)
    if viewer is None:
        print("[WARN] --camera_follow_robot requested, but env_cfg has no viewer config.")
        return
    viewer.origin_type = "asset_root"
    viewer.asset_name = "robot"
    viewer.env_index = 0
    viewer.eye = tuple(float(value) for value in args_cli.camera_eye)
    viewer.lookat = tuple(float(value) for value in args_cli.camera_lookat)
    print(
...
def _robot_root_position(env) -> torch.Tensor | None:
    ...
    if robot is None and hasattr(env, "scene"):
        try:
    ...
    if root_pos is None:
        return None
    try:
        return root_pos[0].detach()
    except Exception:
        return None


def _update_follow_camera(env) -> None:
    """Move the render camera near env0's robot before the next frame is captured."""
    if not args_cli.camera_follow_robot:
        return
    root_pos = _robot_root_position(env)
    if root_pos is None:
        if not getattr(_update_follow_camera, "_warned", False):
            print("[WARN] Robot-follow camera could not find robot root pose; using configured viewer camera only.")
            _update_follow_camera._warned = True
        return
    try:
        eye_offset = torch.tensor(args_cli.camera_eye, device=root_pos.device, dtype=root_pos.dtype)
        lookat_offset = torch.tensor(args_cli.camera_lookat, device=root_pos.device, dtype=root_pos.dtype)
        eye = (root_pos + eye_offset).detach().cpu().tolist()
        target = (root_pos + lookat_offset).detach().cpu().tolist()
        env.sim.set_camera_view(eye=eye, target=target)
    except Exception as exc:
        if not getattr(_update_follow_camera, "_warned", False):
            print(f"[WARN] Robot-follow camera update failed: {exc}")
            _update_follow_camera._warned = True
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    if args_cli.export_policy_only or args_cli.export_policy:
        ...
        print(f"[INFO] {flag} is set. Exiting after export.")
        env.close()
        return
    ...
    _update_follow_camera(unwrapped_env)
    timestep = 0
    frame_count = 0
