...
from .command_safety import resolve_output_selection, validate_enabled_command_payload
...
class MockLowLevelBridge(LowLevelBridgeBase):
    def __init__(self, print_every_n: int = 50) -> None:
        ...
        self.effective_position_rad = [0.0] * 12
        self.effective_velocity_rad_s = [0.0] * 12
    ...
    def send_motor_command(self, cmd) -> None:
        ...
        for index, selected in enumerate(selection.main_drive_enable):
            self.effective_velocity_rad_s[index] = (
                float(cmd.target_velocity_rad_s[index]) if selected else 0.0
            )
            if selected:
                self.effective_position_rad[index] = float(cmd.target_position_rad[index])
        for index in range(6, 12):
            self.effective_velocity_rad_s[index] = (
                float(cmd.target_velocity_rad_s[index])
    ...
    def read_motor_state(self):
        ...
        state.position_rad = list(self.effective_position_rad)
        state.velocity_rad_s = list(self.effective_velocity_rad_s)
