...
class SafetyFilter:
    def __init__(
        ...
        control_deadline_miss_limit: int = 5,
    ) -> None:
        ...
        self.control_deadline_miss_limit = max(int(control_deadline_miss_limit), 1)
        self.control_deadline_miss_count = 0
        self.estop = True
    ...
    def check(
        ...
        if raw_action is not None:
            ...
            # IsaacLab clamps policy outputs in _pre_physics_step(). Some exported
            # policies therefore emit values outside [-1, 1], especially for a
            # zero observation that is far from the normalized training manifold.
            # Do not stop on ordinary >1 raw values; stop only on clearly broken
            # numerics. Decoded targets are checked below after clipping/scaling.
            if np.max(np.abs(raw_action)) > 1.0e6:
                reasons.append("raw action magnitude is numerically unreasonable")
        ...
        if self.last_loop_dt > control_dt * self.control_deadline_factor:
            self.control_deadline_miss_count += 1
            if self.control_deadline_miss_count >= self.control_deadline_miss_limit:
                reasons.append(
                    "control loop deadline miss: "
                    f"{self.last_loop_dt:.4f}s x{self.control_deadline_miss_count}"
                )
        else:
            self.control_deadline_miss_count = 0
