...
- [x] 從 `5de992d` 建立專用 worktree 與分支，並保留所有無關 worktree。
- [x] 在來源 commit 建立 annotated tag `docs-reorg-source-2026-08-01`。
- [x] 保存核准的英文與繁體中文設計配對，以及本實作計畫配對。
- [x] 人工比較每組配對的非在地化 frontmatter、明確錨點 ID、限制、命令與驗收條件。
- [x] 在開始遷移或實作工具前舉行書面規格審查檢查點並記錄核准結果。
...
- [x] 建立雙語操作人員與開發人員入口及核准的中央目錄樹。
- [x] 在治理文件定義 frontmatter schema、列舉值、生命週期與位置規則、命名規則、過時政策及文件影響政策。
- [x] 為每種維護文件類型加入範本，並加入涵蓋意義對等與穩定明確錨點的翻譯指南。
- [x] 定義根目錄與元件使用簡短雙語 `README.md` 的路由慣例。
- [x] 加入根目錄 `AGENTS.md` 作為精簡操作轉接層，引導代理程式使用正式治理規則。
- [x] 加入包含 `@AGENTS.md` 匯入的根目錄 `CLAUDE.md`，讓 Claude Code 接收相同的儲存庫要求。
- [x] 建立標題層級遷移清單，欄位包括舊路徑、舊標題、處置方式、取代文件及移除 commit。
- [x] 遷移或刪除來源檔前，將每份來源文件的每個標題加入清單。

**階段驗收：** 入口與治理文件均已配對並互相連結；schema 範例通過既定規則；路由與翻譯慣例明確無歧義；根目錄代理程式轉接層指向正式治理規則且不複製政策；內容遷移前，每個範圍內的舊標題都有清單紀錄。
...
- [ ] 為兩個技能加入精簡的 `.claude/skills` 包裝檔；各包裝檔引導 Claude Code 使用正式 `.agents/skills` 實作，不複製工作流程政策。
...
- [ ] 測試每個 Claude 包裝路徑都能解析至正式技能檔，且 Claude 遵循正式工作流程。

**階段驗收：** 兩個技能相對無指引對照組均呈現可重現的改善；各自至少有五次微型測試紀錄；每個必要情境類型皆經測試；Claude 包裝路徑能解析並使用正式實作；漏洞修正有文件紀錄；輸出遵守治理規則且未複製政策。
...
- [ ] `AGENTS.md` 指向正式文件治理規則。
- [ ] `CLAUDE.md` 匯入 `AGENTS.md`。
- [ ] 階段 6 完成後，Claude 包裝路徑能解析至正式 `.agents/skills` 檔案。
- [ ] 無論使用何種撰寫工具，不依賴代理程式的驗證都會拒絕不符合規則的輸出。
