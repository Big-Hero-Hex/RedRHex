...
- [x] Mark Windows launcher sessions, route one all-runs TensorBoard through fixed forward `6006`, force headless training, and disable host-only file-manager and live-viewer controls.
- [x] Add browser regression proof for both Windows and macOS markers, fixed-forward TensorBoard, disabled host-only controls, and preserved browser-safe actions.
...
- [ ] Verify the marked panel URL and on-demand TensorBoard after authentication; confirm host-only controls are grey and recorded media remains viewable.
