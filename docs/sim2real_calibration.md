...
Physical torsion-spring imports use `--calibration-constants-json` to bind the declared neutral angle, the exact `damper_0` through `damper_5` representative-alias list, and mechanical-owner approval. The approval identifies the owner and fixture and declares exactly one positive safe envelope: `maximum_safe_deflection_rad`, `maximum_safe_load_n`, or `maximum_safe_torque_nm`. These constants become part of the immutable metadata hash; they are not accepted later as caller-supplied fit results.
...
- `mass-com` requires three repeats and at least three non-collinear planar support positions. Its trace metadata must record all 18 `reference_joint_position_rad` values and a normalized `reference_root_orientation_xyzw` under `calibration_constants`; the holdout must use the same pose. It writes an absolute `target_total_mass_kg` and `reference_planar_com_xy_m`. At runtime Isaac applies and verifies the achieved whole-robot mass and planar CoM; the audit fails if the target, achieved value, or reference pose does not agree.
- Legacy `spring` measurements remain readable for compatibility. New physical calibration uses `torsion-spring`: three signed loading/unloading sweeps at 20/40/60/80% of the approved deflection, load, or torque envelope, followed by a distinct `torsion-spring-holdout` episode at 30/50/70%. The fit must satisfy R² ≥ 0.98, held-out torque RMSE ≤ 5% of full scale, stiffness CV ≤ 5%, hysteresis width ≤ 10% of full scale, and the same held-out RMSE gate with the configured neutral angle. The neutral-constrained stiffness is applied to all six aliases; damping is explicitly overwritten to zero because dynamic damping is unverified. Calibration and holdout episode paths, identities, and hashes remain bound to the source record and are recomputed before a run is stamped calibrated.
...
candidate = apply_measurements_to_profile(
    ...
    trace_paths=[
        ...
        "datasets/sim2real/springs/episodes/damper-0-torsion-calibration",
        "datasets/sim2real/springs/episodes/damper-0-torsion-holdout",
...
The helper accepts only hash-verified real episodes linked by a managed dataset, enforces each reviewed scenario's units, frames, repeat count, and spring envelope levels, and recomputes the metrics itself. It preserves unrelated profile values and records dataset/episode identity, absolute episode path, and actual trace, metadata, and scenario hashes in `measurement_sources`; caller-supplied metrics and hashes are not accepted. It maps mass/CoM to the absolute mass target, neutral-constrained torsion-spring stiffness to all six passive springs, known-load torque to the main-drive effort limit, ABAD results to `hardware_mapping.abad_target_scale` and `abad_target_offset_rad`, and friction to `simulation_physics.ground`. A calibration-only spring profile is intentionally stamped `uncalibrated`; it may be used for a clearly labeled provisional smoke run but cannot pass promotion until its exact holdout evidence is accepted.
...
The evidence binds each real episode and each applicable simulator trace by SHA-256, declares calibration versus holdout conditions and what was held out, records instrument uncertainty, and reports whether bounded actuator candidates entered the real envelope. Executable main-drive step/coast, ABAD, and static-contact holdouts require simulator artifacts. Manual mass/CoM, torsion-spring, and known-load holdouts must not claim an Isaac artifact; the gate instead compares the exact candidate profile target with the managed held-out measurements. Their mandatory metrics are total mass plus planar CoM, neutral-constrained spring stiffness, and positive plus negative saturation torque, respectively. Legacy unsigned `spring` evidence remains loadable for compatibility but is stamped uncalibrated and cannot make a profile eligible for promotion.
