...
class RinboRosBackend(LowLevelBridgeBase):
    def __init__(
        ...
        disabled_legs: Sequence[str] | str | None = None,
    ) -> None:
        ...
        self.disabled_legs = self._parse_disabled_legs(disabled_legs)
    ...
    @staticmethod
    def _parse_disabled_legs(value: Sequence[str] | str | None) -> set[str]:
        if value is None:
            return set()
        ...
        disabled = {item.strip().lower() for item in raw_items if item.strip()}
        unknown = disabled.difference(RINBO_LEG_FIELDS)
        if unknown:
            raise ValueError(f"Unknown rinbo.disabled_legs entries: {sorted(unknown)}")
        return disabled

    def connect(self) -> None:
        self.connected = True
        self.node.get_logger().warn(
            f"BioRoLa/Rinbo backend connected. allow_enable={self.allow_enable}; "
            f"command_topic={self.command_topic}; disabled_legs={sorted(self.disabled_legs)}"
        )
    ...
    def _convert_command(self, cmd, now: float):
        ...
        for rinbo_i, field in enumerate(RINBO_LEG_FIELDS):
            ...
            if field in self.disabled_legs:
                leg.enable = False
                leg.voltage = 0.0
                leg.state = 0
                leg.reset_position = False
        ...
        for rinbo_i, field in enumerate(RINBO_SERVO_FIELDS):
            ...
            leg_field = RINBO_LEG_FIELDS[rinbo_i]
            target_abad = 0.0 if leg_field in self.disabled_legs else abad_rinbo[rinbo_i]
            encoder = int(round(self.servo_zero[rinbo_i] + target_abad * self.servo_counts_per_rad))
    ...
    def diagnostics(self) -> dict[str, str]:
        values = {
            ...
            "disabled_legs": str(sorted(self.disabled_legs)),
