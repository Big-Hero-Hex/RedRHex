...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if np.max(np.abs(action)) > 1.0:
            warnings.append(
                "zero-observation raw action exceeds [-1,1]. This can be normal because IsaacLab clamps actions before applying them; deployment decoder also clips before scaling."
            )
