...
`redrhex_remote.command` 是單一 POSIX shell、自我安裝且以 macOS 為目標的 launcher。它負責 deterministic configuration、desktop installation、endpoint probe、SSH argument、readiness polling 與 browser launch。它不改變 Tailscale、SSH server、Training Panel 內部、Isaac Lab 或 training artifact。
...
`tests/test_redrhex_remote.sh` 會檢查 remote panel command、SSH option/forward ordering、Base64 transport shape、deterministic install path、executable installation 與 byte-for-byte source preservation。執行 dependency-free check：
